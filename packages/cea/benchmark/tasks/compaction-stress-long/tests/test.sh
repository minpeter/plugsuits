...
for f in /agent/work/data/numbers.txt /agent/work/data/words.txt /agent/work/scripts/sum.py /agent/work/scripts/count.py /agent/work/scripts/combine.py; do
    if [ -f "$f" ]; then
        PASS=$((PASS + 1))
    else
        echo "MISSING: $f"
...
if python3 /agent/work/scripts/sum.py 2>&1 | grep -q "Sum:"; then
    PASS=$((PASS + 1))
else
    echo "sum.py failed"
    FAIL=$((FAIL + 1))
...
SUM=$(sed -n '1p' /agent/work/answers.txt | tr -d '[:space:]')
COUNT=$(sed -n '2p' /agent/work/answers.txt | tr -d '[:space:]')
FIRST=$(sed -n '3p' /agent/work/answers.txt | tr -d '[:space:]')
LAST=$(sed -n '4p' /agent/work/answers.txt | tr -d '[:space:]')
