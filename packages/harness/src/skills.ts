...
interface SkillFrontmatter {
  ...
  license?: string;
  metadata?: { author?: string; version?: string; [key: string]: unknown };
  model?: string;
  name?: string;
  triggers?: string[];
  version?: string;
...
// ============================================================================
// Pure Helpers (shared by all methods)
// ============================================================================
...
export class SkillsEngine {
  ...
  async loadAllSkills(): Promise<SkillInfo[]> {
    ...
    const loadBundledLegacy = bundledDir
      ? loadLegacySkills(bundledDir, "bundled")
      : Promise.resolve([] as SkillInfo[]);
    const loadGlobalLegacy = globalSkillsDir
      ? loadLegacySkills(globalSkillsDir, "global")
      : Promise.resolve([] as SkillInfo[]);
    const loadProjectLegacy = projectSkillsDir
      ? loadLegacySkills(projectSkillsDir, "project")
      : Promise.resolve([] as SkillInfo[]);

    const loadV2 = this._loadV2Skills(bundledDir, projectSkillsDir);
    const loadCommands = this._loadAllSlashCommands(
      globalCommandsDir,
      projectCommandsDir
    );
  ...
  async loadSkillsMetadata(): Promise<string> {
    try {
...
`;
  ...
  private async _loadV2Skills(
    bundledPath?: string,
    projectPath?: string
  ): Promise<SkillInfo[]> {
    const globalPath = this.config.globalSkillsDir;

    const [projectDirs, globalDirs, bundledDirs] = await Promise.all([
      projectPath
        ? discoverSkillDirectories(projectPath)
    ...
    for (const skill of allSkills) {
      ...
      let realPathResolved: string;
      try {
        const cached = realpathCache.get(skill.dirPath);
        if (cached) {
          realPathResolved = cached;
        } else {
          realPathResolved = await realpath(skill.dirPath);
          realpathCache.set(skill.dirPath, realPathResolved);
        }
      } catch (error) {
        if (!isExpectedSkillIoError(error)) {
          warnUnexpectedSkillError("_loadV2Skills:realpath", error);
        }
        realPathResolved = skill.dirPath;
      }

      if (seenRealPaths.has(realPathResolved)) {
        continue;
      }
      seenRealPaths.add(realPathResolved);
  ...
  private async _loadAllSlashCommands(
    globalCommandsDir?: string,
    projectCommandsDir?: string
  ): Promise<SkillInfo[]> {
    const [globalCommands, projectCommands] = await Promise.all([
      globalCommandsDir
        ? loadSlashCommands(globalCommandsDir, "global-command")
        : Promise.resolve([] as SlashCommandInfo[]),
      projectCommandsDir
        ? loadSlashCommands(projectCommandsDir, "project-command")
        : Promise.resolve([] as SlashCommandInfo[]),
    ]);
