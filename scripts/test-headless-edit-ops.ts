 ...
 *
 * Usage:
 *   node --import tsx scripts/test-headless-edit-ops.ts [-m <model>] [--provider <provider>]
 */
...
const TEST_CASES: TestCase[] = [
  ...
  },
  {
    ...
    prompt: [
      ...
      "Replace line 2 (tab-indented '\tindented-with-tab') with 4-space-indented '    indented-with-spaces'.",
      "Use edit_file with op='replace', pos=<line2 anchor>, lines=['    indented-with-spaces'].",
    ].join(" "),
    validate: (c) => {
      if (c.includes("\t")) {
  ...
  },
  {
    ...
    validate: (c) => {
      ...
      if (!callLine) {
        return { passed: false, reason: "'new_call' not found" };
      }
      const leadingSpaces =
        callLine.match(LEADING_SPACES_REGEX)?.[0].length ?? 0;
      if (leadingSpaces !== 6) {
        return {
          passed: false,
...
const main = async () => {
  ...
  for (const tc of TEST_CASES) {
    ...
    try {
      ...
      // Ignore cleanup failures between isolated test cases.
    }
  ...
  try {
    ...
    // Ignore cleanup failures at process shutdown.
  }
